...
- **Le point d'extension existe déjà en partie.** `RegleDeResolution` (`packages/sim/src/contacts.ts`) décrit ce qu'un mode fait des contacts relevés, et `regleClassique` en est la première valeur. **Mais `tick` appelle aujourd'hui la règle Classique par défaut**: rien ne transporte encore le mode jusqu'au moteur. Le brancher appartient à l'étape qui ajoutera le champ `mode` à la configuration, 2.4, sans changer le comportement tant qu'un seul mode existe.
