...
| -------------------------------------------------- | ------------------------------------------------------------------------- | ----------------------- |
| **Construire en v1**                               | Mode Classique, devenu la Horde, avec son combo                           | Fait (phase 1, 7.5)     |
|                                                    | Mode Tactique, capture par cône (jalon 5)                                 | Fait (7.1)              |
|                                                    | Mode Équipes, deux équipes d'une couleur chacune (phase 7)                | Fait (7.2)              |
|                                                    | Mode Chasse, traqueurs à trois vies contre proies camouflées (phase 7)    | Fait (7.3)              |
|                                                    | Mode Massacre, katana, combos et sang (phase 7)                           | Fait (7.4)              |
...
| **Reporter entièrement**                           | Les autres modes: Chasse, Battle Royale, Chaos                            | Après le socle (6.1)    |
...
| ------------ | ------------------------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `mode`       | `'classique'`, `'tactique'`, `'equipes'`, `'chasse'` ou `'massacre'` | `classique` | Le Tactique depuis l'étape 7.1, les Équipes depuis la 7.2, la Chasse depuis la 7.3, le Massacre depuis la 7.4. **Figé à la création**: changer de mode pourrait changer la capacité, et exclure des joueurs déjà présents. |
...
| capacité     | entier, déduit du mode     | 12          | **Pas un champ**: une propriété du mode. Classique et Tactique: 12 au plus, 1 pour lancer. Équipes: 12 au plus, six par équipe, et un joueur dans chaque équipe pour lancer. Chasse: 10 au plus, 2 pour lancer, et pas d'entrée une fois lancée. Massacre: 8 au plus, 1 pour lancer. |
...
- **Un mode peut aussi changer ce qui n'est pas un contact ni une entrée.** Élargissement de l'étape 7.2: le jeu de règles dit désormais aussi ce qu'un joueur perd quand un bot noir l'attrape, et qui subit un malus ramassé. Ces deux décisions vivaient dans le comportement des bots et dans le ramassage des objets, sans consulter le mode; le Classique et le Tactique y gardent exactement le code d'avant, ce que l'empreinte des parties vérifie.
- **Une équipe, pour le moteur, c'est une couleur** (étape 7.2). Le mode Équipes n'ajoute aucun champ à l'état: deux joueurs de même couleur sont coéquipiers, le score d'une équipe est le nombre de bots de sa couleur, et l'interdiction de capturer un joueur de sa propre couleur vaut déjà. Qui est dans quelle équipe se décide au salon, par le serveur, qui fait entrer chaque joueur avec la couleur de la sienne; `packages/shared/src/equipes.ts` fait le passage entre couleur et équipe pour le serveur comme pour le client.
- **Un mode peut imposer ses réglages, décider d'une fin avant le terme et mettre un joueur hors jeu** (étape 7.3). La Chasse n'a pas de Black Ninjas, quoi que l'hôte règle (`imposerLesReglagesDuMode`, appliquée par le moteur à la création de l'état). Le jeu de règles dit aussi ce que le mode fait au lancement (`lancer`: la Chasse y tire ses traqueurs), s'il est décidé avant le terme (`estDecidee`: plus de proie, ou plus de traqueur en jeu), et qui est hors jeu (`horsJeu`: un traqueur éliminé ne bouge plus, ne ramasse rien, ne subit aucun malus, et disparaît du flux). Les trois autres modes n'y font rien, sans aucun tirage: l'empreinte des parties Classique est identique.
- **Le rôle d'un joueur de la Chasse vit dans l'état, et se lit à la couleur** (étape 7.3). `EtatPartie.chasse` porte les traqueurs (vies, orientation, attente du tir) et le parcours des proies; une seule fonction fait d'un joueur un traqueur et lui donne la couleur réservée des traqueurs, que le client lit pour savoir qui est qui. L'arme d'un traqueur voyage dans l'état tactique du flux: ses charges sont ses vies.
- **Le Massacre se branche sans élargir le jeu de règles** (étape 7.4). Il frappe dans `agir`, pose ses armes et ses points au lancement (`lancer`, `EtatPartie.massacre`), se décide quand la carte est vidée (`estDecidee`), et retire la zone de chaos (`imposerLesReglagesDuMode`). Le cône du Tactique est devenu réglable pour son arc de katana. Le sang, les traces de pas et la traînée des coups vivent dans la page: le moteur n'annonce que les morts.
- **Le Classique devient la Horde, avec un combo** (étape 7.5). Le nom change pour le joueur, pas l'identifiant `classique`, que gardent les parties et records enregistrés. Le combo se branche sans élargir le jeu de règles: la Horde pose ses combos au lancement (`EtatPartie.horde`), les laisse s'épuiser dans `agir`, et rallie dans sa règle de contacts (`regleHorde`), qui distingue le contact d'un joueur de la contagion entre ninjas. La règle du multiplicateur (`COMBO`, `multiplicateurDuCombo`) est commune au Massacre et à la Horde. Joueurs, bots et Black Ninjas vont tous à 150 pixels par seconde hors bonus, dans tous les modes.
