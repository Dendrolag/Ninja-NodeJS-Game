...
pnpm charge               # harnais de charge du serveur (--banc, --melange, --reseau, --rapide, --mode tactique ou equipes, --sortie fichier.json)
...
Base de données (depuis l'étape 3.1): PostgreSQL sur Neon, projet `neon-ninja`. Trois variables d'environnement, jamais commitées: `DATABASE_URL`, l'adresse par le pooler; `NEON_API_KEY` et `NEON_PROJECT_ID`, pour les branches de test. Les deux dernières sont aussi des secrets du dépôt GitHub. Sans elles, les tests de la base sont sautés en local et font échouer la CI. Le jeu, lui, tourne sans base, en invités seulement; avec `DATABASE_URL`, le serveur branche les comptes (étape 3.2: inscription, connexion, session par jeton), leur progression de fin de partie (étape 3.3) et la gestion du mot de passe (étape 3.4: changement depuis le profil, code de secours pour un mot de passe oublié), sur une base migrée par `pnpm base:migrer`.
...
1. Le score est un **stock, pas un cumul**: se faire capturer le remet à zéro. C'est ce qui fait la tension de fin de partie. Précision du 16 septembre 2026 (étape 7.3): en mode Chasse, les ninjas ne font pas le score; une proie marque en parcourant la carte et garde ses points quand elle est infectée, un traqueur marque par ses captures et ses vies restantes. En mode Massacre (étape 7.4), les points se gagnent en tuant et se rangent dans l'état: se faire tuer par un joueur en cède la moitié à son tueur. Précision du 18 septembre 2026 (étape 7.5): le Classique s'appelle désormais la **Horde** (identifiant `classique` inchangé); les ninjas ralliés à la suite y font un combo, dont la prime s'ajoute au score dans une réserve **qui se perd avec les ninjas à la capture**, sans aller au capteur: le score reste un stock. Précision du 25 septembre 2026 (étape 7.9): le x2 de l'Évadé double le score de qui le porte, et en Équipes celui de toute son équipe; il ne se range pas, seul le porteur l'est, et le score se déduit comme avant, puis double.
2. Capturer un joueur transfère **tous** ses bots d'un coup. Précision du 16 septembre 2026 (étape 7.3): en mode Chasse, capturer une proie ne transfère rien, elle devient traqueur. En mode Massacre (étape 7.4), on ne capture plus: un coup de katana tue. Depuis l'étape 7.9, capturer ou tuer le porteur du x2 de l'Évadé transfère aussi le x2; un Black Ninja qui l'attrape le détruit.
3. Un bot noir fait perdre 50 pour cent des points; un bot noir détruit en rapporte 15. Précision du 16 septembre 2026 (étape 7.4): en mode Massacre, ces 15 points sont multipliés par le combo, et seul le katana détruit un bot noir. En Horde (étape 7.5), la réserve de primes de combo perd la même part que les ninjas, et le combo retombe; les 15 points ne sont pas multipliés.
4. Un malus ramassé frappe **les autres**, pas celui qui le ramasse. Précision du 16 septembre 2026 (étape 7.2): en mode Équipes, « les autres » sont l'équipe adverse, et les coéquipiers du ramasseur sont épargnés. En mode Chasse (étape 7.3), « les autres » sont l'autre camp, et un traqueur éliminé n'est pas frappé. Le Classique, le Tactique et le Massacre (étape 7.4) frappent tous les autres joueurs, sans changement: seul en Massacre, un malus ne frappe personne.
5. Vitesses relatives: joueur 3, bot 5, **bot noir 5**, bonus de vitesse x1,7, facteur mobile x2. Déjà corrigé quatre fois dans le legacy, très fragile. Correction du 13 août 2026: ce fichier et l'audit annonçaient 6 pour le bot noir. C'est faux. `BlackBot` lit `GAME_CONFIG.BOT_SPEED`, qui vaut 5, et le réglage `blackBotSpeed: 6` n'est lu nulle part. Vérifié et couvert par les tests de caractérisation. Décision du 13 août 2026: on garde 5, la vitesse réellement jouée depuis deux ans. Précision du 13 août 2026 (étape 1.1): ces nombres sont des distances par pas, et les pas n'ont pas la même durée. Un joueur avance de 3 toutes les 20 millisecondes, un bot de 5 toutes les 50: ramenés à la seconde, **le joueur va à 150 pixels par seconde et le bot à 100**, donc le joueur est le plus rapide. Ce sont ces vitesses par seconde que le moteur applique. Détail au défaut X17 de l'audit. Précision du 15 septembre 2026: le facteur mobile n'est pas porté; sur téléphone, le legacy allait réellement à 120 pixels par seconde (6 pixels toutes les 50 millisecondes), et le porteur du projet a choisi la même vitesse sur tous les appareils. **Changement du 18 septembre 2026 (étape 7.5), sur décision du porteur du projet: une vitesse commune de 150 pixels par seconde** aux joueurs, aux bots et aux bots noirs, hors bonus, dans tous les modes. Le bot ne va donc plus à 100: un bot noir lancé ne se distance plus sans bonus de vitesse. Les tests de caractérisation, qui observent le legacy, figent toujours 3 et 5 par pas.
...
11. **Les bots se transmettent leur couleur au contact, mais seule une couleur de joueur se transmet.** Un troupeau rouge qui traverse un troupeau bleu le retourne. Un bot blanc, lui, ne repeint rien: il n'a pas de couleur à donner. Le legacy ne posait aucune condition, si bien que le blanc effaçait les scores par simple diffusion. Corrigé le 14 août 2026 sur décision du porteur du projet, défauts X20 et X30 de l'audit. C'est le seul écart de gameplay volontaire du portage. Précision du 14 septembre 2026 (étape 5.4): comme dans le legacy, les bots naissent d'une couleur tirée au sort, qui n'est celle d'aucun joueur; « couleur de joueur » veut dire la couleur d'un joueur présent dans la partie, si bien qu'un bot de couleur quelconque, repeint par une zone de chaos ou resté à la couleur d'un joueur parti ne repeint rien non plus. Défaut X36 de l'audit.
